...
// // static void some_smp_function(struct bt_conn *conn, const uint8_t *ltk) {
// // LOG_INF("LTK is being used!");
// // LOG_HEXDUMP_INF(ltk, 16, "LTK Value:"); // Print LTK in hexadecimal format
// // }
...
//  Log the LTK value in HEX
//     LOG_INF("LTK found for connection: %p", conn);
//     LOG_HEXDUMP_INF(conn->le.keys->ltk, 16, "LTK Value:");
// 	return true;
// }
...
// 	LOG_INF("Received paring confirm value:");
// 	// #################jasmine
// 	LOG_DBG("");
...
// 	LOG_DBG("");
// 	LOG_INF("RECEIVED PAIRING RANDOM VALUE");
// 	// #############jASMINEEEEEEEEEEEEEEEEEEEE
...
static uint8_t smp_dhkey_generate(struct bt_smp *smp)
{
    ...
    LOG_INF("DHKey generation successful");
	return 0;
}
...
void bt_smp_update_keys(struct bt_conn *conn)
{
	...
	if (atomic_test_bit(smp->flags, SMP_FLAG_SC)) {
		...
		if (atomic_test_bit(smp->flags, SMP_FLAG_BOND)) {
			...
			LOG_INF("LTK Stored: 0x%s", bt_hex(conn->le.keys->ltk.val, sizeof(conn->le.keys->ltk.val)));
			// ####################JASMINE
			(void)memset(conn->le.keys->ltk.rand, 0,
				     sizeof(conn->le.keys->ltk.rand));
			(void)memset(conn->le.keys->ltk.ediv, 0,
